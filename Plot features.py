...
for col in adni_df.columns[:-1]: # Exclude the Status variable
    fig, ax = plt.subplots()
    sns.histplot(data=adni_df, x=col, hue='Posttreatment Subcutaneous fat', kde=True, ax=ax)
...
for col in adni_df.columns[:-1]: # Exclude the Status variable
    fig, ax = plt.subplots()
    sns.boxplot(data=adni_df, x='Posttreatment Subcutaneous fat', y=col, ax=ax)
...
sns.pairplot(adni_df, hue='Posttreatment Subcutaneous fat')
...
for col in adni_df.columns[:-1]: # Exclude the Status variable
    fig, ax = plt.subplots()
    sns.violinplot(data=adni_df, x='Posttreatment Subcutaneous fat', y=col, ax=ax)
...
for col in adni_df.columns[:-1]: # Exclude the Status variable
    fig, ax = plt.subplots()
    sns.scatterplot(data=adni_df, x=col, y='Posttreatment Subcutaneous fat', ax=ax)
    ax.set_title(f"Scatter plot of {col} vs. Posttreatment Subcutaneous fat")
